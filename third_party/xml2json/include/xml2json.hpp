...
void xml2json_traverse_node(rapidxml::xml_node<> *xmlnode, rapidjson::Value &jsvalue, rapidjson::Document::AllocatorType& allocator)
{
    ...
    if((xmlnode->type() == rapidxml::node_type::node_data || xmlnode->type() == rapidxml::node_type::node_cdata) && xmlnode->value())
    {
        // case: pure_text
        jsvalue.SetString(xmlnode->value(), allocator);  // then addmember("#text" , jsvalue, allocator)
    }
    else if(xmlnode->type() == rapidxml::node_type::node_element)
    {
        if(xmlnode->first_attribute())
        {
            if(xmlnode->first_node() && xmlnode->first_node()->type() == rapidxml::node_type::node_data && count_children(xmlnode) == 1)
            {
        ...
        else
        {
            ...
            else if(xmlnode->first_node()->type() == rapidxml::node_type::node_data && count_children(xmlnode) == 1)
            {
                // case: <e>text</e>
                if (xml2json_numeric_support == false)
                {
        ...
        if(xmlnode->first_node())
        {
            ...
            for(xmlnode_chd = xmlnode->first_node(); xmlnode_chd; xmlnode_chd = xmlnode_chd->next_sibling())
            {
                ...
                rapidjson::Value jn, jv;
                if(xmlnode_chd->type() == rapidxml::node_type::node_data || xmlnode_chd->type() == rapidxml::node_type::node_cdata)
                {
                ...
                else if(xmlnode_chd->type() == rapidxml::node_type::node_element)
                {
...
bool xml2jsondoc(rapidjson::Document *js_doc, const char *xml_str) {
    //file<> fdoc("track_orig.xml"); // could serve another use case
    rapidxml::xml_document<> *xml_doc = new rapidxml::xml_document<>();

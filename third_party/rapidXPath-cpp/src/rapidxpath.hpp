...
std::any XPath(rapidxml::xml_node<char> *node, std::string querry){
	...
	while(true){
		...
		if(querry.size() == 0){
			if(newnode.size() == 1)
				return std::any(newnode[0]);
			else if (newnode.size() == 0)
				return std::any( int(-1));
			else
				return std::any( std::vector<rapidxml::xml_node<>*>(newnode));
		}
		...
		if(subquerry == "text" || subquerry[0] == '@'){
			...
			if(subquerry == "text"){
				...
				switch(str.size()){
					case 0: return std::any( int(-1));
					case 1: return std::any( std::string(str[0]));
					default: return std::any( std::vector<std::string>(str));
				}
			} else if(subquerry[0] == '@'){
				for(int i = 0; i < newnode.size(); i++){
			...
			switch(str.size()){
				case 0: return std::any( int(-1)); break;
				case 1: return std::any( string(str[0])); break;
				default: return std::any( std::vector<std::string>(str));
			}	
		}
